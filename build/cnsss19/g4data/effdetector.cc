...
void effdetector(){
	...
	TFile *file = new TFile("data_Co60_6cluster.root","read");
	TTree *data = (TTree *)file -> Get("data");
//	TFile *file1 = new TFile("hist_Co_ringtype.root","recreate");
...
//--------------add back-------------------
	for(int i =0; i< nent; i++)
	{
		data->GetEntry(i);
		if(i==0) a=eventID;
		cout << eventID << " "<< volumeID <<" "<< KineticEnergy<<endl;
...
//--------------------------------------------------------
	...
	{
		ergeff_ab[i] = 100*(hist_24det->GetBinContent(i*1000+1000)+hist_24det->GetBinContent(i*1000+1001))/(nevt/12);
		graph_ab->SetPoint(i, 100*i+100, ergeff_ab[i]);
	}
	for (Int_t i = 3; i < 5; i++)
	{
		ergeff_ab[i+7] = 100*(hist_24det->GetBinContent(i*5000)+hist_24det->GetBinContent(i*5000+1))/(nevt/12);
	...
	for (Int_t i = 0; i < 10; i++)
	{       
		ergeff[i] = 100*(hist_onedet->GetBinContent(i*1000+1000)+hist_onedet->GetBinContent(i*1000+1001))/(nevt/12);
		graph->SetPoint(i, 100*i+100, ergeff[i]);
	}
	for (Int_t i = 3; i < 5; i++)
	{       
		ergeff[i+7] = 100*(hist_onedet->GetBinContent(i*5000)+hist_onedet->GetBinContent(i*5000+1))/(nevt/12);
